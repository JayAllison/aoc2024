...
# break the input into the two separate sections at the blank line, so that we can parse them separately
...
# in other words, every page in that list (the dict value) must be after the page that is the dict key
# this makes it easy to build the algorithm below around the 'in' operator
#
...
# we could convert these number strings to ints here, but we don't have to yet, so I didn't
for rule in rules_lines:
    first, second = rule.split('|')
    rules[first].append(second)

# -----------------------------------------------------------------------------
# Parsing the updates section:
# -----------------------------------------------------------------------------
...
for pages in updates_section.split('\n'):
    # there may be one blank line at the end - we want to skip it, so check each time that the line isn't blank
    if pages:
...
for good in good_updates:
    ...
    # also, this is the only place we must convert the number string to an integer, so do that now
