# -----------------------------------------------------------------------------
# This is very similar to Day 16 but slightly different:
#  - the Dijkstra implementation today is simpler
#  - we don't have a map, per se, but we do have points to avoid
# -----------------------------------------------------------------------------
...
corruptions: list = incoming_bytes[:simulated_steps]  # Part 1: only process so many of the points provided in the input
unvisited_nodes: list[tuple] = [(x, y) for x, y in product(range(MAX_X+1), range(MAX_Y+1)) if (x, y) not in corruptions]
...
while True:
    ...
    # for Part 1, we know there is a solution, so we don't need to account for not finding a suitable path
    if current_node == end:
        print(scores[current_node])
        break
